...
import { TeamClient } from "@/app/settings/team/team-client";
...
function AccessPageInner() {
  ...
  const [tab, setTab] = useState<"members" | "project" | "user" | "roles">(
    initialUser ? "user" : initialProject ? "project" : "members"
  );
  ...
  return (
    <>
      ...
      {/* Members — team list + invite (merged from the old Team page) */}
      {tab === "members" && <TeamClient />}

      {/* Selector */}
      {(tab === "project" || tab === "user") && (
        <div className="mb-6">
          {tab === "project" ? (
